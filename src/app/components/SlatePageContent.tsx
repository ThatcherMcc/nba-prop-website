...
import type { TodaysPlayer, TopPick, TeamDefensiveRating } from "@/lib/data";
...
function PlayerRow({
  ...
  return (
    <div
      className={`flex items-center py-2 px-3 border-b border-pe-border/5 last:border-0 hover:bg-pe-surface-2/20 transition-colors ${
        hasEdge ? "bg-emerald-500/[0.03]" : ""
      }`}
    >
      {/* Left: Name + stats, fixed layout */}
      <div className="flex items-center gap-x-2 min-w-0">
        <Link
          href={`/player/${encodeURIComponent(player.playerName)}`}
          className="text-[13px] font-semibold text-pe-text-primary hover:text-pe-accent transition-colors truncate w-[110px] sm:w-[130px] shrink-0"
        >
          {player.playerName}
        </Link>
        <span className="hidden sm:block text-[13px] font-mono text-pe-text-primary tabular-nums w-[38px] text-right shrink-0">
          {player.ptsLine ?? "—"}
        </span>
        <span className="hidden sm:block text-[13px] font-mono text-pe-text-primary tabular-nums w-[32px] text-right shrink-0">
          {player.rebLine ?? "—"}
        </span>
        <span className="hidden sm:block text-[13px] font-mono text-pe-text-primary tabular-nums w-[32px] text-right shrink-0">
          {player.astLine ?? "—"}
      ...
      {/* Right: Edge badges, pushed to far right */}
      {hasEdge && (
...
function TeamColumn({
  ...
  return (
    <div className="flex flex-col min-w-0">
      ...
      {/* Column headers */}
      <div className="hidden sm:flex items-center gap-x-2 px-3 py-0.5 border-b border-pe-border/5">
        <span className="text-[10px] font-bold text-pe-text-faint uppercase tracking-wider w-[130px] shrink-0">Player</span>
        <span className="text-[10px] font-bold text-pe-text-faint uppercase tracking-wider w-[38px] text-right shrink-0">PTS</span>
        <span className="text-[10px] font-bold text-pe-text-faint uppercase tracking-wider w-[32px] text-right shrink-0">REB</span>
        <span className="text-[10px] font-bold text-pe-text-faint uppercase tracking-wider w-[32px] text-right shrink-0">AST</span>
        <span className="text-[10px] font-bold text-pe-text-faint uppercase tracking-wider ml-auto">Edge</span>
      </div>
...
export default function SlatePageContent({
  ...
  const edgeCount = new Set(topPicks.map((p) => p.playerName)).size;
      ...
      <div className="flex items-center gap-3 py-1.5 px-3 mb-4 bg-pe-surface-1 border border-pe-border/10 rounded-lg text-[11px] font-mono text-pe-text-faint">
        ...
        {edgeCount > 0 && (
          <>
            ...
            <span><span className="text-emerald-400 font-bold">{edgeCount}</span> edges</span>
          </>
        )}
      ...
      {!hasGames ? (
        <div className="bg-pe-surface-1/60 border border-pe-border/10 rounded-2xl p-12 text-center">
          ...
          <p className="text-pe-text-faint text-sm">
            Check back on game days for today&apos;s full slate with prop lines and edges.
          </p>
          <Link
            href="/analytics"
